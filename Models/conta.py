...
class Conta:
    ...
    def depositar(self: object, valor: float) -> None:
        if valor > 0:
            ...
            # acessa setter = acessa property e adiciona valor
            self.saldo_total = self._calcula_saldo_total
            # acessa setter = acessa a property e adiciona valor
            print('Depósito efetuado com sucesso!')
        else:
            print('Erro ao efetuar depósito. Tente novamente.')

    def sacar(self: object, valor: float) -> None:
        if not valor < 0 and valor < self.saldo_total:
            if self.saldo >= valor:
                self.saldo = self.saldo - valor
                self.saldo_total = self._calcula_saldototal
            else:
    ...
    def transferir(self: object, destino: object, valor: float) -> None:
        # Sai de um objeto Conta para um destino obejto Conta
